...
public class SphereMouseLook : MonoBehaviour {
    ...
    public void LookRotation(Transform character, Quaternion characterTargetRot, Transform camera) {
        float yRot = CrossPlatformInputManager.GetAxis("Mouse X") * XSensitivity;
        float xRot = CrossPlatformInputManager.GetAxis("Mouse Y") * YSensitivity;

        Quaternion charRot = characterTargetRot * Quaternion.Euler(0f, yRot, 0f);
        ...
        if (smooth) {
            character.localRotation = Quaternion.Slerp(character.localRotation, charRot,
                smoothTime * Time.deltaTime);
            camera.localRotation = Quaternion.Slerp(camera.localRotation, m_CameraTargetRot,
                smoothTime * Time.deltaTime);
        } else {
            character.localRotation = charRot;
